...
NAMESPACESCHANNEL = {
    'channel': 'http://www.domain.org/restfulapi/2011/channels-xml'}
...
def Start():
    ...
    # Da sollte noch ein "initial Channel/EPG" und eine Art "Close Stream"
    # hook hin, oder ?
...
def MainMenu():
    ...
    if Prefs['host'] and Prefs['port'] and Prefs['stream']:
        ...
        Log("Loading VDR GroupsList via restfulapi-plugin at Port %s" %
            (Prefs['restapi']))

        numberOfGroups = int(
            GroupList.xpath(
                '//group:count/text()',
                namespaces=NAMESPACESGROUP)[0])

        item = 0
        for item in range(numberOfGroups):
            # jeden Namen einzeln holen aus dem XPath Object statt aus der
            # Liste
            groupName = GroupList.xpath(
                '//group:group/text()',
                namespaces=NAMESPACESGROUP)[item]
            groupListContainer.add(
                DirectoryObject(
                    key=Callback(
...
def DisplayGroupChannels(name):
    ...
    xml = SINGLEGROUP_URL % (
        Prefs['host'], Prefs['restapi'], groupNameURLconform)
    try:
    ...
    numberOfChannels = int(
        channelGroupList.xpath(
            '//channel:count/text()',
            namespaces=NAMESPACESCHANNEL)[0])

    channelListContainer = ObjectContainer(
        title2=name,
        view_group='List',
        no_cache=True)
    ...
    for item in range(numberOfChannels):
        ...
        Channel_Name = channelGroupList.xpath(
            '//channel:param[@name="name"]/text()',
            namespaces=NAMESPACESCHANNEL)[item]
        Channel_ID = channelGroupList.xpath(
            '//channel:param[@name="channel_id"]/text()',
            namespaces=NAMESPACESCHANNEL)[item]
        hasChannelLogo = channelGroupList.xpath(
            '//channel:param[@name="image"]/text()',
            namespaces=NAMESPACESCHANNEL)[item]

        channelListContainer.add(
            LiveTVMenu(
                sender=Channel_Name,
                channel=Channel_ID,
                thumb=hasChannelLogo))
...
def LiveTVMenu(sender, channel, thumb, include_oc=False):
    ...
    currentEpgXml = CHANNELEPG_CURRENT_URL % (
        Prefs['host'], Prefs['restapi'], channel, "1")
    ...
    #try:
    #    currentEvent = currentEpg.xpath('//epg:param[@name="id"]/text()',
    #                                    namespaces=NAMESPACEEPG)
    # except:
    # 	Log("VDR Plugin: Couldn't get EPG")
    try:
        currentTitle = currentEpg.xpath(
            '//epg:param[@name="title"]/text()',
            namespaces=NAMESPACEEPG)[0]
        # currentEpgImage = CHANNELEPG_IMAGE_URL % (Prefs['host'],
        #                            Prefs['restapi'], currentEvent, "0")
    except:
        currentTitle = "no data"
    try:
        currentDescription = currentEpg.xpath(
            '//epg:param[@name="description"]/text()',
            namespaces=NAMESPACEEPG)[0]
    except:
        currentDescription = "no data"
    try:
        # currentSubtitle = currentEpg.xpath('//epg:param[@name="short_text"]/text()',
        #                                        namespaces=NAMESPACEEPG)[0]
        currentStartTime = currentEpg.xpath(
            '//epg:param[@name="start_time"]/text()',
            namespaces=NAMESPACEEPG)[0]
    except:
        currentStartTime = "0"
    try:
        currentDuration = currentEpg.xpath(
            '//epg:param[@name="duration"]/text()',
            namespaces=NAMESPACEEPG)[0]
    except:
        currentDuration = "0"

    stream = STREAM_URL % (Prefs['host'],
                           Prefs['port'],
                           Prefs['stream'],
                           channel)
